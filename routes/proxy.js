...
async function safeFetch(targetUrl, allowlist) {
  ...
  for (let hop = 0; hop <= config.proxy.maxRedirects; hop += 1) {
    ...
    // DNS再解決による検査後の宛先すり替えを防ぐ。Host/SNIはURLのまま維持。
    const pinned = validation.resolvedIps[0];
    const dispatcher = new Agent({connect:{lookup:(_host,_opts,cb)=>cb(null,pinned,net.isIP(pinned))}});
...
router.get('/', requireAuth, proxyRateLimit, async (req, res) => {
  const now=Date.now();
  if(req.session.proxyLast && now-req.session.proxyLast>300000){req.session.proxyLast=null;return res.status(408).json({error:'5分間操作がなかったためタイムアウトしました'});}
  req.session.proxyLast=now;
