...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const joinCode = isPrivate ? crypto.randomBytes(9).toString('base64url') : null;
    const joinHash = joinCode ? crypto.createHash('sha256').update(joinCode).digest('hex') : null;
    if (typeof name !== 'string' || !name.trim()) {
      return res.status(400).json({ error: 'ルーム名を入力してください' });
    }
    const result = await db.query(
      'INSERT INTO rooms (name, is_private, owner_id, join_code_hash) VALUES ($1, $2, $3, $4) RETURNING id, name, is_private',
      [name.trim().slice(0, 64), Boolean(isPrivate), req.session.userId, joinHash]
    );
    await db.query("INSERT INTO room_members (room_id, user_id, role) VALUES ($1, $2, 'owner')", [
      result.rows[0].id,
      req.session.userId,
    ]);
    res.status(201).json({ room: result.rows[0], joinCode });
  } catch (err) {
...
router.post('/:id/join', requireAuth, async (req, res) => {
  try {
    ...
    const room = await db.query('SELECT is_private,join_code_hash FROM rooms WHERE id = $1', [roomId]);
    if (room.rows.length === 0) return res.status(404).json({ error: 'ルームが見つかりません' });
    if (room.rows[0].is_private && !isAdminUser(req.session.user)) {
      const code = String((req.body||{}).joinCode||'');
      const hash = crypto.createHash('sha256').update(code).digest('hex');
      if (!code || hash !== room.rows[0].join_code_hash) return res.status(403).json({ error: '参加コードが正しくありません' });
    }
    if (!isAdminUser(req.session.user) && await isBanned(roomId, req.session.userId)) {
...
router.post('/:id/kick', requireAuth, async (req, res) => {
  try {
    ...
    const target = await db.query('SELECT username,is_admin FROM users WHERE id=$1',[targetUserId]);
    if (isAdminUser(target.rows[0])) return res.status(403).json({ error: '管理者は対象にできません' });
...
router.post('/:id/messages', requireAuth, async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO chat_logs(room_id,user_id,username,message,action) VALUES($1,$2,$3,$4,$5)',[roomId,req.session.userId,message.username,safeContent,'send']);
...
router.delete('/messages/:id', requireAuth, async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO chat_logs(room_id,user_id,username,message,action) VALUES($1,$2,$3,$4,$5)',[message.room_id,req.session.userId,req.session.user.username,String(messageId),'delete']);
    await db.query('INSERT INTO admin_logs(admin_id,action,target) VALUES($1,$2,$3)',[req.session.userId,'message_delete',String(messageId)]);
