...
app.get("/api/youtube/stream",async(req,res)=>{
  ...
  if(!/^https?:\\/\\/(www\\.)?(youtube\\.com|youtu\\.be)\\//i.test(source))
    return res.status(400).json({error:"YouTube URLを指定してください"});
  try{
    const args=[
      "--no-playlist",
      "--no-warnings",
    ...
    const {stdout}=await execFileAsync("yt-dlp",args,{timeout:30000,maxBuffer:1024*1024*4});
    const streamUrl=stdout.trim().split(/\\r?\\n/).filter(Boolean).pop();
    if(!streamUrl)return res.status(404).json({error:"ストリームURLを取得できませんでした"});
    res.json({
      url:streamUrl,
      note:"取得した一時ストリームURLは期限切れになる場合があります。"
    });
  }catch(e){
    res.status(502).json({error:"yt-dlpでストリームを取得できませんでした"});
  }
});
